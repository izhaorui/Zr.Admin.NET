...
- ⚠️ 并行分叉(7)的出边**不要带条件**：在 AI 生成模型下，并行分叉(7)/并行汇聚(8) 的 parallelGroup 会被置为 0（fork/join 不带组号、走引擎 fork 网关路径），若分支带条件，不满足的成员不会生成任务、汇聚网关会傻等它而卡死。若描述要求「某条件才走某并行分支」，应改用**条件网关(4)** 选路：满足条件 → 引入并行分叉段；不满足 → 走另一分支/默认边。并行分叉(7) 只负责无条件并发激活全部分支。
